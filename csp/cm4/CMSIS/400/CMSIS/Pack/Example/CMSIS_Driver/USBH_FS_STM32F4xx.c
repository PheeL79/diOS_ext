...
/**
  \fn          uint32_t USBH_HW_CH_GetIndexFromAddress (OTG_FS_HC *ptr_ch)
  \brief       Get the Index of Channel from it's Address.
  \param[in]   ptr_ch   Pointer to the Channel
  \return      Index of the Channel
*/
__INLINE static uint32_t USBH_HW_CH_GetIndexFromAddress (OTG_FS_HC *ptr_ch) {
  return (ptr_ch - (OTG_FS_HC *)(&(OTG->HCCHAR0)));
}

/**
  \fn          OTG_FS_HC *USBH_HW_CH_GetAddressFromIndex (uint32_t index)
  \brief       Get the Channel Address from it's Index.
  \param[in]   index    Index of the Channel
  \return      Address of the Channel
*/
__INLINE static OTG_FS_HC *USBH_HW_CH_GetAddressFromIndex (uint32_t index) {
...
*/
__INLINE static bool USBH_HW_CH_Disable (OTG_FS_HC *ptr_ch) {
  int i;
...
/**
  ...
  \return      true = success, false = fail
*/
static bool USBH_HW_CH_TransferEnqueue (OTG_FS_HC *ptr_ch, uint32_t packet, uint8_t *data, uint32_t num) {
  uint32_t  hcchar;
  uint32_t  hctsiz;
  uint32_t  hcintmsk;
  uint32_t  mpsiz;
...
static int32_t USBH_HW_Initialize (ARM_USBH_SignalPortEvent_t cb_port_event, ARM_USBH_SignalEndpointEvent_t cb_endpoint_event) {
  ...
  if (OTG_FS_PinsConfigure (ARM_USB_PIN_DP | ARM_USB_PIN_DM | ARM_USB_PIN_OC | ARM_USB_PIN_VBUS) == false) return ARM_DRIVER_ERROR;
  ...
  OTG->HNPTXFSIZ = ((512/4)<<16) | ((512/4)  +4);   /* Non-peri TxFIFO        */
...
static int32_t USBH_HW_Uninitialize (void) {
  ...
  if (OTG_FS_PinsUnconfigure (ARM_USB_PIN_DP | ARM_USB_PIN_DM | ARM_USB_PIN_OC | ARM_USB_PIN_VBUS) == false) return ARM_DRIVER_ERROR;
...
static int32_t USBH_HW_PortVbusOnOff (uint8_t port, bool vbus) {
  ...
  if (vbus) {                                       /* VBUS power on          */
    ...
    if (OTG_FS_PinVbusOnOff (true ) == false) return ARM_DRIVER_ERROR;
    osDelay(200);                                   /* Allow VBUS to stabilize*/
    OTG->HAINT    =  0x000000FF;                    /* Clear port interrupts  */
    OTG->GINTSTS  =  0xFFFFFFFF;                    /* Clear interrupts       */
    OTG->GAHBCFG |=  OTG_FS_GAHBCFG_GINTMSK;        /* Disable interrupts     */
  } else {                                          /* VBUS power off         */
    if (OTG_FS_PinVbusOnOff (false) == false) return ARM_DRIVER_ERROR;
...
static int32_t USBH_HW_EndpointTransferAbort (ARM_USBH_EP_HANDLE ep_hndl) {
  ...
  ch_idx = USBH_HW_CH_GetIndexFromAddress ((OTG_FS_HC *)ep_hndl);

  if (transfer_info[ch_idx].status.active) {
    transfer_info[ch_idx].status.active = 0;
    if (!USBH_HW_CH_Disable((OTG_FS_HC *)(ep_hndl))) return ARM_DRIVER_ERROR;
  }
...
static int32_t USBH_HW_EndpointTransfer (ARM_USBH_EP_HANDLE ep_hndl, uint32_t packet, uint8_t *data, uint32_t num) {
  ...
  USBH_HW_CH_TransferEnqueue ((OTG_FS_HC *)ep_hndl, packet, data, num);
...
static uint32_t USBH_HW_EndpointTransferGetResult (ARM_USBH_EP_HANDLE ep_hndl) {
  ...
  return (transfer_info[USBH_HW_CH_GetIndexFromAddress((OTG_FS_HC *)ep_hndl)].transferred);
}
...
void USBH_FS_IRQ (uint32_t gintsts) {
  ...
  uint32_t  *dfifo;
  uint32_t   hprt, haint, hcint, pktcnt, mpsiz, hcchar, hcchar_upd;
