...
static void USBD_EndpointReadSet (uint8_t ep_addr) {
  ...
  if (OTG_EP_OUT_TYPE(ep_addr) == ARM_USB_ENDPOINT_ISOCHRONOUS) {
    if ((USBD_GetFrameNumber() & 1)) OTG_DOEPCTL(ep_addr) |= OTG_HS_DOEPCTLx_SEVNFRM;
...
static void USBD_WriteToFifo (uint8_t ep_addr) {
  ...
  if (OTG_EP_IN_TYPE(ep_num) == ARM_USB_ENDPOINT_ISOCHRONOUS) {
    if (USBD_GetFrameNumber() & 1) OTG_DIEPCTL(ep_num) |= OTG_HS_DIEPCTLx_SEVNFRM;
...
static int32_t USBD_Initialize (ARM_USBD_SignalDeviceEvent_t   cb_device_event,
  ...
  if (OTG_HS_PinsConfigure (ARM_USB_PIN_DP | ARM_USB_PIN_DM) == false) return ARM_DRIVER_ERROR;
...
static int32_t USBD_Uninitialize (void) {
  ...
  if (OTG_HS_PinsUnconfigure (ARM_USB_PIN_DP | ARM_USB_PIN_DM) == false) return ARM_DRIVER_ERROR;
...
static int32_t USBD_EndpointConfigure (uint8_t ep_addr,
  ...
  if ((RTE_USB_OTG_HS_DEV_EP & (1 << num)) == 0) return ARM_DRIVER_ERROR;
  ...
  if (dir) {
    ...
    if (OTG_EP_IN_TYPE(num) == ARM_USB_ENDPOINT_ISOCHRONOUS) {
      ...
      if (USBD_GetFrameNumber() & 1) OTG_DIEPCTL(num) |= OTG_HS_DIEPCTLx_SEVNFRM;
      ...
      if ((msk >> num) & 1) {
        ...
        if (ep_int & OTG_HS_DOEPINTx_EPDISD) {
          if (OTG_EP_OUT_TYPE(num) == ARM_USB_ENDPOINT_ISOCHRONOUS) {
            ...
            // Set correct frame
            if ((USBD_GetFrameNumber() & 1)) OTG_DOEPCTL(num) |= OTG_HS_DOEPCTLx_SEVNFRM;
  ...
  if (gintsts & OTG_HS_GINTSTS_EOPF) {
    for (num = 1; num <= USBD_EP_NUM; num++) {
      ...
      if (OTG->GINTSTS & OTG_HS_GINTSTS_IPXFR) {
        if ((USBD_GetFrameNumber() & 1) == ((OTG_DOEPCTL(num) >> OTG_HS_DOEPCTLx_EONUM_POS) & 1)) {
...
// Incomplete isochronous IN transfer
  if (gintsts & OTG_HS_GINTSTS_IISOIXFR) {
    ...
    for (num = 1; num <= USBD_EP_NUM; num++) {
      ...
      if (OTG_DIEPCTL(num) & OTG_HS_DIEPCTLx_EPENA) {
        if ((USBD_GetFrameNumber() & 1) == ((OTG_DIEPCTL(num) >> OTG_HS_DIEPCTLx_EONUM_POS) & 1)) {
