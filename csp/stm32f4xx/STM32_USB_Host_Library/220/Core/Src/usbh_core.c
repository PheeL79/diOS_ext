...
#endif
#if (USBH_USE_diOS == 1)
    OS_SignalSend(usbhd_stdin_qhd, OS_ISR_SignalCreate(DRV_ID_USBH, OS_SIG_USBH_EVENT_PORT, phost->id), OS_MSG_PRIO_NORMAL);
#endif
  return USBH_OK;
}
...
#endif
#if (USBH_USE_diOS == 1)
    OS_SignalSend(usbhd_stdin_qhd, OS_ISR_SignalCreate(DRV_ID_USBH, OS_SIG_USBH_EVENT_STATE_CHANGED, phost->id), OS_MSG_PRIO_NORMAL);
#endif
      }
    }
    break;
